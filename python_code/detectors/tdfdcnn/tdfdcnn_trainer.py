...
from python_code.detectors.tdfdcnn.tdfdcnn_detector import TDFDCNNDetector
...
class TDFDCNNTrainer(Trainer):
    ...
    def _initialize_detector(self, num_bits, n_users, n_ants):
        ...
        self.detector = [[TDFDCNNDetector(num_bits, n_users).to(DEVICE) for _ in range(conf.iterations)] for _ in
                         range(n_users)]  # 2D list for Storing the TDFDCNN Networks
    ...
    def _train_model(self, single_model: nn.Module, tx: torch.Tensor, rx_prob: torch.Tensor, num_bits:int, epochs: int, first_half_flag: bool) -> list[float]:
        """
        Trains a TDFDCNN Network and returns the total training loss.
        """
    ...
    def _train_models(self, model: List[List[TDFDCNNDetector]], i: int, tx_all: List[torch.Tensor],
    ...
    def _online_training(self, tx: torch.Tensor, rx_real: torch.Tensor, num_bits: int, n_users: int, iterations: int, epochs: int, first_half_flag: bool, probs_in: torch.Tensor):
        """
        Main training function for TDFDCNN trainer. Initializes the probabilities, then propagates them through the
        ...
        # Training the TDFDCNN network for each user for iteration=1
        ...
        # Training the TDFDCNN for each user-symbol/iteration
        for i in range(1, iterations):
            # Training the TDFDCNN networks for the iteration>1
