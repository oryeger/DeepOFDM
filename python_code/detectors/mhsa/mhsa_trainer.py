...
from python_code.detectors.mhsa.mhsa_detector import MHSADetector
...
class MHSATrainer(Trainer):
    ...
    def _initialize_detector(self, num_bits, n_users, n_ants):
        ...
        self.detector = [[MHSADetector(num_bits, n_users).to(DEVICE) for _ in range(conf.iterations)] for _ in
                         range(n_users)]  # 2D list for Storing the MHSA Networks
    ...
    def _train_model(self, single_model: nn.Module, tx: torch.Tensor, rx_prob: torch.Tensor, num_bits:int, epochs: int, first_half_flag: bool) -> list[float]:
        """
        Trains a MHSA Network and returns the total training loss.
        """
    ...
    def _train_models(self, model: List[List[MHSADetector]], i: int, tx_all: List[torch.Tensor],
    ...
    def _online_training(self, tx: torch.Tensor, rx_real: torch.Tensor, num_bits: int, n_users: int, iterations: int, epochs: int, first_half_flag: bool, probs_in: torch.Tensor):
        """
        Main training function for MHSA trainer. Initializes the probabilities, then propagates them through the
