% =========================================================
% master_plot_mi_bler_lmmse_compare.m
%   MI (left) + BLER (right) side-by-side for two LMMSE runs:
%   lmmse0.mat  and  lmmse50.mat  (in the same folder)
% =========================================================

clear; clc;

% ---- User configuration ----
base_name        = 'transfer_64QAM';
extra_text       = '';             % e.g. '_transfer'
root_dir         = 'C:\Projects\Scratchpad\mat_files\';

algs_to_plot     = [1 2];         % 1=lmmse0, 2=lmmse50
add_snr_target   = false;         % append SNR@10% to BLER legend labels
plot_aug_iter_2  = false;         % plot second aug iteration if available
snr_pad_left_db   = 0;            % extend SNR axis to the left by this many dB
snr_cut_right_pts = 0;            % cut this many SNR points from the right
snr_cut_left_pts  = 0;            % cut this many SNR points from the left
output_target    = 'paper';       % 'paper' or 'ppt'
% ----------------------------
...
fprintf('Plotting MI + BLER (LMMSE0 vs LMMSE50) from: %s\n', dir_path);
% -------------------------------------------

% ---- Shared style definitions ----
% Two distinct colors for the two LMMSE variants
alg_colors = [0.00, 0.45, 0.70;   % LMMSE0  - blue
              0.85, 0.33, 0.10];  % LMMSE50 - vermillion

alg_names = {'LMMSE0', 'LMMSE50'};
alg_files = {'lmmse0', 'lmmse50'};
...
% Reorder so row 1 = all no-aug, row 2 = all aug.
desired_order = {'LMMSE0',     'LMMSE50', ...
                 'LMMSE0 aug', 'LMMSE50 aug'};
reorder_idx = [];
...
legend_args = {'Interpreter', 'none', 'Orientation', 'horizontal', 'NumColumns', 2};
if is_ppt
...
% ---- Export ----
out_name = fullfile(root_dir, [base_name, extra_text, '_mi_bler_lmmse_compare']);
