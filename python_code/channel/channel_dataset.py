...
from python_code.utils.constants import NUM_BITS, BLOCK_LENGTH_FACTOR, N_USERS
...
class ChannelModelDataset(Dataset):
    ...
    def __init__(self, block_length: int, pilots_length: int, blocks_num: int, num_res: int, fading_in_channel: bool, spatial_in_channel: bool, delayspread_in_channel: bool, clip_percentage_in_tx: int, cfo: int, go_to_td: bool, cfo_in_rx: bool, kernel_size: int):
        """
        ...
        self.channel_type = MIMOChannel(self.block_length, pilots_length, fading_in_channel, spatial_in_channel, delayspread_in_channel, clip_percentage_in_tx, cfo, go_to_td, cfo_in_rx)
    ...
    def get_snr_data(self, snr: float, database: list):
        ...
        for index in range(self.blocks_num):
            tx, h, rx, rx_ce, s_orig = self.channel_type._transmit_and_detect(snr, self.num_res, index)
            # accumulate
            tx_full[index] = tx
            rx_full[index] = rx
    ...
    def __getitem__(self, snr_list: List[float]) -> Tuple[torch.Tensor, torch.complex, torch.complex]:
        database = []
