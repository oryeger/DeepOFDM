...
class DeepRxTrainer(Trainer):
    ...
    def _train_model(self, single_model: nn.Module, tx: torch.Tensor, rx: torch.Tensor, num_bits: int, epochs: int, first_half_flag: bool, network_id: str = "") -> list[float]:
        """
        ...
        tag = f"[DeepRx {network_id}]" if network_id else "[DeepRx]"
        if log_enabled:
