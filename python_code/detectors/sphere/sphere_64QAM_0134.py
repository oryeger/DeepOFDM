...
def Sphere64qam0134(
    ...
    *,
    ...
    """
    Optimized 64-QAM sphere decoder that outputs LLRs only for bits 0, 1, 3, 4.

    We group the 64 constellation points into 16 "super-symbols" based on (b0, b1, b3, b4).
    Bits 2 and 5 are treated as "don't care" and are completed by ESCNN or another method.

    Returns:
        LLRs_all: (n_symbols * 4, n_users) - LLRs for bits 0, 1, 3, 4
        hard_bits_all: (n_symbols * 4, n_users) - hard decisions for bits 0, 1, 3, 4
    """
    ...
    bps = 6
    M = 64
    kout = 4  # output bits: 0, 1, 3, 4
    keep_bits = np.array([0, 1, 3, 4], dtype=np.int32)
    ...
    # Group 64 points by (b0, b1, b3, b4) into 16 groups
    # Each group has 4 points (for b2, b5 = 00, 01, 10, 11)

    # Map from 4-bit label (b0, b1, b3, b4) to super-symbol index
    def bits_to_super_idx(b0, b1, b3, b4):
        return b0 * 8 + b1 * 4 + b3 * 2 + b4
    ...
    for i in range(64):
        b = bits_table_64[i]
        sidx = bits_to_super_idx(b[0], b[1], b[3], b[4])
    ...
    for sidx in range(16):
        b0, b1, b3, b4 = super_idx_to_bits(sidx)
        bits_table_4[sidx, :] = [b0, b1, b3, b4]
    ...
    def enumerate_super_indices(est, level, partial_dist, radius_sq):
        ...
        Returns (super_indices, best_sub_indices).
        """
    ...
    # print(f"Sphere64qam0134: {t_total:.3f}s total | {t_total/max(n_symbols,1)*1e3:.2f} ms/sym")

    if debug:
        print(f"[DEBUG] Sphere64qam0134 total={t_total:.3f}s "
