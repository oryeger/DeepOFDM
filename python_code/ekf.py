...
bits (scored, for the data region; trained on, for the sgdbcei calib region -
...
here - see config.yaml's weights_track_mode comment):
  ...
  'sgdsyn' - blind tsyn loss (its L_tent/L_synd terms use no ground truth - see
  ...
  'sgdbcei' - BCE (or any other supervised loss) trained against a separate
      ...
      on data neither 'ekf' nor 'sgdsyn'/'sgdbce' has seen either. That calib
...
def _build_ekf_filename_suffix(chan_text: str, mod_text: str, n_users: int, code_rate) -> str:
    ...
    _cfo_str = '0' if conf.cfo == 0 else f'{conf.cfo:.2f}'
    _cfod_val = getattr(conf, 'cfo_drift', 0.0)
    _cfod_str = '0' if _cfod_val == 0 else f'{_cfod_val:.2f}'
    title_string = (f"{chan_text}_sp={conf.speed}_{mod_text}_RE={conf.num_res}_UEs={n_users}"
                     f"_an={conf.n_ants}_cfo={_cfo_str}_cfod={_cfod_str}"
...
def _build_dmrs_bce_training_data(rx_dmrs: torch.Tensor, known_tx: dict, layout: dict, H: torch.Tensor,
    ...
    """Build (tx_labels, rx_real, probs_prior, real_bit_idx) for weights_track_mode='sgdbce':
...
def _transmit_and_estimate(content: dict, n_ants: int, num_res: int, n_users: int, h: np.ndarray,
    ...
    complex - weights_track_mode='sgdbce' trains directly on these (see
...
def run_group(escnn_trainer: ESCNNTrainer, codec: LDPC5GCodec, crc: CRC5GCodec, rng: np.random.Generator,
    ...
    if weights_track_mode == 'ekf':
        ...
        if escnn_frozen:
            ...
            # scoring, no separate calib region generated/transmitted at all (see module
            # docstring). tx_data_t is passed through only for the constant-bit mask and
            # collapse diagnostics, never as supervision. payload_symbols_per_slot=
            # _DMRS_NUM_PAYLOAD_SYMB (not a full 14-symbol slot) is also what tells
            # _train_model there's no primary-detector split and no held-out validation to
            # apply here - see its docstring.
            tx_data_t = torch.from_numpy(tx_bits.astype(np.float32))
