...
def Sphere64qam1245(
    ...
    *,
    ...
    """
    Optimized 64-QAM sphere decoder that outputs LLRs only for bits 1, 2, 4, 5.

    We group the 64 constellation points into 16 "super-symbols" based on (b1, b2, b4, b5).
    Bits 0 and 3 are treated as "don't care" and are completed by ESCNN or another method.

    Returns:
        LLRs_all: (n_symbols * 4, n_users) - LLRs for bits 1, 2, 4, 5
        hard_bits_all: (n_symbols * 4, n_users) - hard decisions for bits 1, 2, 4, 5
    """
    ...
    bps = 6
    M = 64
    kout = 4  # output bits: 1, 2, 4, 5
    keep_bits = np.array([1, 2, 4, 5], dtype=np.int32)
    ...
    # Group 64 points by (b1, b2, b4, b5) into 16 groups
    # Each group has 4 points (for b0, b3 = 00, 01, 10, 11)

    # Map from 4-bit label (b1, b2, b4, b5) to super-symbol index
    def bits_to_super_idx(b1, b2, b4, b5):
        return b1 * 8 + b2 * 4 + b4 * 2 + b5
    ...
    for i in range(64):
        b = bits_table_64[i]
        sidx = bits_to_super_idx(b[1], b[2], b[4], b[5])
    ...
    for sidx in range(16):
        b1, b2, b4, b5 = super_idx_to_bits(sidx)
        bits_table_4[sidx, :] = [b1, b2, b4, b5]
