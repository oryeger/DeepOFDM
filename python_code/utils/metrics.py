...
def calculate_ber_even(prediction: torch.Tensor, target: torch.Tensor, num_bits: int) -> float:
    """
    Returns the calculated ber of the prediction and the target (ground truth transmitted word)
    """
    prediction = prediction.long()
    ...
    ber = 1 - torch.mean(torch.all(prediction[::2,:] == target[::2,:], dim=1).float()).item()
    return ber

def calculate_ber_odd(prediction: torch.Tensor, target: torch.Tensor, num_bits: int) -> float:
    """
    Returns the calculated ber of the prediction and the target (ground truth transmitted word)
    """
    prediction = prediction.long()
