...
Expects the per-group lines ekf.py's run_streaming_drift() prints, e.g.:
    [drift] 1428 groups x 1 slot(s)/group (12/14 payload symbols/slot, 2 DMRS),
        track_mode=ekf, which_augment=AUGMENT_LMMSE, starting at channel_drift_base_index=0,
...
HEADER_RE = re.compile(
    ...
    # "(P/T payload symbols/slot, D DMRS)" - added alongside embedded-pilot DMRS layout;
    # optional so logs from before that still parse.
    r'(?:\s+\((?P<payload_symb>\d+)/(?P<slot_symb>\d+) payload symbols/slot,\s+(?P<dmrs>\d+) DMRS\))?'
    # Old-style "+ N calib slot(s)/group" (pre-DMRS-layout logs) vs. new-style
        ...
        for line in f:
            ...
            if header_m:
                ...
                if meta is None:
                    ...
                    calib_slots = header_m.group('calib_slots_old') or header_m.group('calib_slots_new')
                    if calib_slots is not None:
                        meta['calib_slots'] = int(calib_slots)
